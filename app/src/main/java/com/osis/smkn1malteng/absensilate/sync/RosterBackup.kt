 ...
 * - Roster hanya berisi data identitas siswa (nama, kelas, jurusan, No HP, No HP orang tua).
...
object RosterBackup {
    ...
    private val header = "Nama,Kelas,Jurusan,NoHP,NoHP_Ortu\n"
                ...
                students.forEach { student ->
                    writer.write(
                        "${escape(student.name)}," +
                            ...
                            "${student.jurusan.display}," +
                            "${escape(student.phone)}," +
                            "${escape(student.parentPhone ?: "")}\n"
                    )
                }
                writer.flush()
            }
    ...
    fun parseRosterFromUri(contentResolver: ContentResolver, uri: Uri): List<StudentEntity> {
        ...
        for ((index, line) in lines.withIndex()) {
            ...
            try {
                ...
                val jurusanStr = parts.getOrNull(2)?.trim() ?: ""
                val phone = parts.getOrNull(3)?.trim() ?: ""
                val parentPhone = parts.getOrNull(4)?.trim()?.ifEmpty { null }

                // 🔥 STRICT enum matching — nilai tidak dikenal → baris dilewati dengan log
                val kelas = StudentClass.values().firstOrNull {
                ...
                val jurusan = StudentMajor.values().firstOrNull {
                    it.display == jurusanStr || it.name == jurusanStr
                }
                if (jurusan == null) {
                    Log.w("RosterBackup", "Line ${index + 1}: jurusan tidak dikenali '$jurusanStr' — baris dilewati")
                    continue
                }
