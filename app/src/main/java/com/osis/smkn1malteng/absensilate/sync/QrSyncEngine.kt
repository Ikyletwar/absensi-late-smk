...
object QrSyncEngine {
    ...
    private data class StudentQrData(
        ...
        val kelas: String,          // display name, misal "X"
        val jurusan: String,        // display name, misal "TJKT 1"
    ...
    suspend fun decodeStudentsLegacy(encoded: String): List<StudentEntity> = withContext(Dispatchers.IO) {
        try {
            ...
            qrDataList.mapNotNull { qrData ->
                try {
                    val kelas = StudentClass.values().firstOrNull { it.display == qrData.kelas }
                        ?: StudentClass.X
                    val jurusan = StudentMajor.values().firstOrNull { it.display == qrData.jurusan }
                        ?: StudentMajor.TJKT_1
                    StudentEntity(
                        uuid = UUID.randomUUID().toString(),
                        name = qrData.name,
                        phone = qrData.phone,
                        parentPhone = qrData.parentPhone,
                        kelas = kelas,
                        jurusan = jurusan,
    ...
    private fun mergeStudentsInternal(existing: StudentEntity, incoming: StudentEntity): StudentEntity {
        ...
        val finalParentPhone = incoming.parentPhone ?: existing.parentPhone

        return existing.copy(
            phone = finalPhone,
            parentPhone = finalParentPhone,
            kelas = incoming.kelas,
            jurusan = incoming.jurusan,
