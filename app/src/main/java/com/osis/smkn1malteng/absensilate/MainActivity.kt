...
fun App() {
    ...
    // ============================================
    // ============================================
    // 🔥 LAUNCHER UNTUK EXPORT CSV (manual & weekly reset)
    // ============================================
    var weeklyExportPending by remember { mutableStateOf(false) }

    val exportLauncher = rememberLauncherForActivityResult(
        contract = ActivityResultContracts.CreateDocument("text/csv")
    ) { uri ->
        if (weeklyExportPending) {
            weeklyExportPending = false
            uri?.let {
                scope.launch {
                    if (viewModel.exportCsvSuspend(it, context)) {
                        viewModel.resetWeeklyAfterExport()
                    }
                }
            }
        } else {
    ...
    if (uiState.showWeeklyReset) {
        WeeklyResetModal(
            weekNumber = uiState.currentWeek,
            year = uiState.currentYear,
            onExportReset = {
                val fileName = "Data_Terlambat_Minggu_${uiState.currentWeek}.csv"
                weeklyExportPending = true
                exportLauncher.launch(fileName)
            },
            onForceReset = { viewModel.performForceReset() },
    ...
    if (uiState.showParentPhoneBottomSheet && uiState.selectedStudent != null) {
        ParentPhoneBottomSheet(
            studentName = uiState.selectedStudent!!.name,
            onSave = { phone, ctx -> viewModel.saveParentPhoneAndSendWa(phone, ctx) },
            onDismiss = { viewModel.clearViolationState() },
            context = context
        )
    }
    ...
    if (!uiState.showWeeklyReset) {
        if (uiState.showRecapScreen) {
            RecapScreen(
                ...
                onExportCsv = {
                    val uri = RecapExporter.exportRecapCsv(context, viewModel.getSelectedStudentsForRecap())
                    if (uri != null) {
                ...
                onExportPdf = {
                    val uri = RecapExporter.exportRecapPdf(context, viewModel.getSelectedStudentsForRecap())
                    if (uri != null) {
            ...
            InputFormScreen(
                ...
                initialParentPhone = editingStudent?.parentPhone ?: "",
                initialKelas = editingStudent?.kelas ?: StudentClass.X,
                initialJurusan = editingStudent?.jurusan ?: StudentMajor.TJKT_1,
                initialTimestamp = System.currentTimeMillis(),
                isEditMode = isEditMode,
                onSave = { name, phone, parentPhone, kelas, jurusan, timestamp ->
                    if (isEditMode && editingStudent != null) {
                        ...
                        viewModel.addStudent(name, phone, parentPhone, kelas, jurusan, timestamp)
                    }
                    showForm = false
                },
            ...
            DashboardScreen(
                ...
                onDeleteClick = { id -> viewModel.deleteStudent(id) },
                ...
                onToggleSelection = { viewModel.toggleStudentSelection(it) },
                selectedStudentIds = uiState.selectedStudentIds,
                onOpenRecap = { viewModel.openRecapScreen() },
                maxViolation = uiState.maxViolation,
                onMaxViolationChange = { viewModel.updateMaxViolation(it) },
                viewModel = viewModel
            )
        }
    }
