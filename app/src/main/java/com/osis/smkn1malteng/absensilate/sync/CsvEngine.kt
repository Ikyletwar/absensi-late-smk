...
object CsvEngine {
    ...
    private val header = "ID,Nama,NoHP,NoHP_Ortu,Kelas,Jurusan,ViolationCount,TotalPelanggaran,Timestamps\n"
            ...
            InputStreamReader(inputStream).buffered().use { reader ->
                reader.forEachLine { line ->
                    ...
                    try {
                        ...
                        val parentPhone = parts[3].trim().ifEmpty { null }
                        val kelasStr = parts[4].trim()
                        val jurusanStr = parts[5].trim()
                        ...
                        // Mapping display to enum
                        val kelas = StudentClass.values().firstOrNull { it.display == kelasStr }
                            ?: StudentClass.X
                        val jurusan = StudentMajor.values().firstOrNull { it.display == jurusanStr }
                            ?: StudentMajor.TJKT_1
