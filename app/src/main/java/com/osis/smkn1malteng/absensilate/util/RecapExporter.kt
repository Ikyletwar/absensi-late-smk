...
object RecapExporter {
        ...
        return try {
            ...
            uri?.let { outputUri ->
                context.contentResolver.openOutputStream(outputUri)?.bufferedWriter()?.use { writer ->
                    // Header
                    writer.write("No,Nama,Kelas,Jurusan,NoHP,NoHP_Ortu,PelanggaranMingguIni,TotalPelanggaran,RiwayatTimestamp\n")
    ...
    fun exportRecapPdf(
        ...
        // === HEADER SEKOLAH ===
        canvas.drawText("SMKN 1 MALUKU TENGAH", MARGIN, yPos + 16f, headerPaint)
        yPos += 24f
        ...
        for ((index, student) in students.withIndex()) {
            ...
            // Kelas & Jurusan
            canvas.drawText("   Kelas: ${student.kelas.display} ${student.jurusan.display}", MARGIN, yPos + 10f, bodyPaint)
            yPos += LINE_HEIGHT
        ...
        canvas.drawLine(MARGIN, yPos, PAGE_WIDTH - MARGIN, yPos, dividerPaint)
        yPos += 12f
        canvas.drawText("Dicetak oleh: Aplikasi Absensi Late SMKN 1 Maluku Tengah", MARGIN, yPos + 8f, smallPaint)
